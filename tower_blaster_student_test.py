...
class Test_tower_blaster(unittest.TestCase):
    ...
    def test_deal_initial_bricks(self):
        ...
        # note some values from the main pile prior to dealing, so we can use these values for testing later
        first_brick = main_pile[0]
        second_brick = main_pile[1]
        nineteenth_brick = main_pile[18]
        twentieth_brick = main_pile[19]
        ...
        # check that a new brick is placed on top (beginning of the list)
        self.assertEqual(computer_bricks[0], nineteenth_brick)
        self.assertEqual(user_bricks[0], twentieth_brick)
        self.assertEqual(computer_bricks[-1], first_brick)
        self.assertEqual(user_bricks[-1], second_brick)
